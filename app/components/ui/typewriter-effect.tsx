...
export const TypewriterEffect = ({
  ...
  }, [isInView, animate]); // 添加 animate 作为依赖
        ...
        <div key={`word-${idx}`} className="inline-block">
          {word.text.map((char, index) => (
            <motion.span
              ...
              key={`char-${index}`}
              className={cn(
                `dark:text-gray-400 text-black opacity-0 hidden`,
                word.className
              )}
            >
              {char}
  ...
  return (
    <div className={cn("text-base sm:text-sm md:text-sm lg:text-5xl font-bold text-center", className)}>
      {renderWords()}
      <motion.span
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
